...
import { DataType as DataTypeRooms } from "../../rooms/page-components/form-components";
...
export const useFetchAvailable = (queryParams:string,customReference = "getavailable") => {
    const queryClient = useQueryClient();

    const fetch = async () => {
        return (
            await axios.get<(DataTypeWithId & {id?:string,rooms:DataTypeRooms[]})[] >(
                `${SERVER_ROUTE}${MAIN_ROUTE}/available${queryParams}`
            )
        ).data;
    };
    const { data, isFetching } = useQuery([customReference], fetch);
...
export const useFetchRecommended = (customReference = "getrecommended") => {
    const queryClient = useQueryClient();

    const fetch = async () => {
        return (
            await axios.get<DataTypeWithId[]>(
                `${SERVER_ROUTE}${MAIN_ROUTE}/recommended`
            )
        ).data;
    };
    const { data, isFetching } = useQuery([customReference], fetch);
